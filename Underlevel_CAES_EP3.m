function [Cost_1,P_CAES_c_sum] = Underlevel_CAES_EP3(x,st,ss)    
%下层日前优化调度程序：场景A
...
Cst=[Cst,P_LW_error./vf_lim - K_WS_2.*sum(P_wind,1) <= 2*sum(u_G.*repmat(P_G_max.*H_G,1,Horizon),1) + ...
    sum(u_CAES_d.*repmat(P_CAES_d_max.*H_CAES,1,Horizon),1) + K_WS_2.*sum(u_CAES_c.*repmat(P_CAES_c_max,1,Horizon),1)];
...
%系统稳态频差计算
df_steady = P_LW_error./(k_load*sum(P_load,1) +  k_load*sum(P_CAES_c,1) + K_WS_1.*sum(P_wind,1) + ...
    sum(u_G.*repmat(P_G_max./adj_G,1,Horizon),1) + sum(u_CAES_d.*repmat(P_CAES_d_max./adj_CAES,1,Horizon),1));
df_steady_0 = P_LW_error./(k_load*sum(P_load,1) + K_WS_1.*sum(P_wind,1) + ...
    sum(u_G.*repmat(P_G_max./adj_G,1,Horizon),1));
df_steady = 50.*value(df_steady);
df_steady_0 = 50.*value(df_steady_0);
%系统频率变化率计算
vf = P_LW_error./(2*sum(u_G.*repmat(P_G_max.*H_G,1,Horizon),1) + ...
    ...
    K_WS_2.*sum(u_CAES_c.*repmat(P_CAES_c_max,1,Horizon),1));
vf_0 = P_LW_error./(2*sum(u_G.*repmat(P_G_max.*H_G,1,Horizon),1) + ...
    K_WS_2.*sum(P_wind,1));
vf = 50.*value(vf);
vf_0 = 50.*value(vf_0);
%数据整理
...
CAES_data_da = value([u_CAES_c;u_CAES_d;P_CAES_c;P_CAES_d;...
    CAES_pst;R_CAES_up_c+R_CAES_up_d;R_CAES_down_c+R_CAES_down_d;...
    R_CAES_em_c+R_CAES_em_d+R_CAES_em_s]);                           %用于展示CAES启停计划、出力情况、备用情况
...
%% 数据输出
filename = 'D:\文章-大规模CAES多阶段优化规划-程序\算例EP3.xlsx';
xlswrite(filename,Gen_data_da(1:15,:),'火电调度计划','B3:Y17');
xlswrite(filename,Gen_data_da(16:30,:),'火电调度计划','B21:Y35');

xlswrite(filename,CAES_data_da,'CAES调度计划','B2:Y57');

xlswrite(filename,net_load_1,'净负荷','B3:Y3');
xlswrite(filename,net_load_2,'净负荷','B8:Y8');

xlswrite(filename,0.05.*sum(P_load,1),'系统备用-爬坡需求','B3:Y3');
xlswrite(filename,R_nl_up,'系统备用-爬坡需求','B8:Y8');
xlswrite(filename,R_nl_dn,'系统备用-爬坡需求','B9:Y9');

xlswrite(filename,vf,'频率指标情况','B3:Y3');
xlswrite(filename,df_steady,'频率指标情况','B13:Y13');
xlswrite(filename,vf_0,'频率指标情况','B4:Y4');
xlswrite(filename,df_steady_0,'频率指标情况','B14:Y14');
